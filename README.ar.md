...
تأتي الواجهة بـ**الإنجليزية** (الافتراضية) و**العربية** (من اليمين إلى اليسار) و**الإسبانية** و**الصينية المبسطة**. تُحدَّد لغة الطلب بهذا الترتيب: معامل `?culture=` في الرابط، ثم كوكي الثقافة القياسي في ASP.NET Core، ثم ترويسة `Accept-Language` للمتصفح، ثم الافتراضية. يكتب مبدّل اللغة في الشريط العلوي (وعلى صفحة تسجيل الدخول) هذا الكوكي ثم يعيد التحميل.

لا يحتاج التطبيق المضيف إلى أي إعداد: يُدخِل الحزمة وسيط الترجمة الخاص بها مقتصرًا على مسارات الواجهة فقط (`/login` وصفحات الواجهة و`/api/yarp/*`)، فلا يحتاج المضيف إلى استدعاء `UseRequestLocalization` وتبقى صفحاته الخاصة بالسلوك الثقافي الذي ضبطها عليه.

تتحكم إعدادان في مجموعة اللغات (في `appsettings.json`):

| الإعداد | الافتراضي | المعنى |
| --- | --- | --- |
| `YarpUi:Cultures` | `en,ar,es,zh-Hans,zh-CN` | ثقافات مفصولة بفواصل يجوز للواجهة الرد بها |
| `YarpUi:DefaultCulture` | `en` | الثقافة المستخدمة عندما لا يطابق الطلب أي ثقافة مدعومة |

يُقبَل `zh-CN` كاسم بديل لـ `zh-Hans` (المتصفحات ترسل الوسم الإقليمي)؛ وتتراجع الثقافات غير المدعومة إلى الافتراضية. وتُترجَم رسائل الأخطاء — من واجهة الإدارة ومن فحوص صحة الإعدادات في المحرّر — بالثقافة نفسها للطلب.
